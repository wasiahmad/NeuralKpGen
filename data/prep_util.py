...
class MultiprocessingTokenizer(object):
    ...
    def process(self, example):
        ...
        title_tokenized = title_tokenized.replace('[ digit ]', constants.DIGIT)
        abstract_tokenized = abstract_tokenized.replace('[ digit ]', constants.DIGIT)
        pkp_tokenized = [pkp.replace('[ digit ]', constants.DIGIT) for pkp in pkp_tokenized]
        akp_tokenized = [akp.replace('[ digit ]', constants.DIGIT) for akp in akp_tokenized]
...
def fn_replace_digits(text, tokenizer='wordpunct'):
    ...
    for tok in in_tokens:
        if re.match('^\d+$', tok):
            out_tokens.append(constants.DIGIT)
        else:
