...
from deepkp.utils.misc import aeq
...
class CopyGeneratorCriterion(object):
    ...
    def __call__(self, scores, align, target):
        ...
        align_unk = align.eq(constants.UNK).to(dtype=scores.dtype)
        align_not_unk = align.ne(constants.UNK).to(dtype=scores.dtype)
        target_unk = target.eq(constants.UNK).to(dtype=scores.dtype)
        target_not_unk = target.ne(constants.UNK).to(dtype=scores.dtype)
