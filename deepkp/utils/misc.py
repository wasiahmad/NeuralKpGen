...
def tens2sen(t, word_dict=None, src_vocabs=None):
    ...
    for idx, s in enumerate(t):
        ...
        if len(sentence) == 0:
            ...
            sentence = [str(constants.PAD_WORD)]
...
def tens2sen_score(pred, prob, word_dict=None, src_vocabs=None, ignore_empty=False):
    sentences = []
    scores = []
    ...
    for idx in range(batch_size):
        sentence = []
        score = []
        for widx in range(len(pred[idx])):
            word = pred[idx][widx]
            ws = prob[idx][widx]
            if word in [constants.BOS]:
                continue
        ...
        if len(sentence) == 0 and not ignore_empty:
            # NOTE: just a trick not to score empty sentence
            # this has no consequence
            sentence = [constants.PAD_WORD]
            score = [0.0]
