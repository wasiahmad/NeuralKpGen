...
import numpy as np
import sys
from deepkp.inputters.constants import PAD, PAD_WORD, UNK, UNK_WORD, \
    BOS, BOS_WORD, EOS, EOS_WORD, PKP_BOS, PKP_BOS_WORD, AKP_BOS, AKP_BOS_WORD
...
class Vocabulary(object):
    def __init__(self, no_special_token=False):
        ...
        if not no_special_token:
            self.tok2ind[BOS_WORD] = BOS
            self.tok2ind[EOS_WORD] = EOS
            self.tok2ind[PKP_BOS_WORD] = PKP_BOS
            self.tok2ind[AKP_BOS_WORD] = AKP_BOS
            self.ind2tok[BOS] = BOS_WORD
            self.ind2tok[EOS] = EOS_WORD
            self.ind2tok[PKP_BOS] = PKP_BOS_WORD
            self.ind2tok[AKP_BOS] = AKP_BOS_WORD
    ...
    def load(self, filename, vocab_size=None):
